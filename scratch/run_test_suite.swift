...
"""
        ...
        assert(config?.nodes.first?.type == .shadowsocks, "Type", "First node should be shadowsocks")
        assert(config?.nodes.last?.type == .trojan, "Type", "Second node should be trojan")
    }
...
"""
        ...
        assert(config?.nodes.first?.name == "Inline SS", "Name", "Should parse inline name correctly")
...
"""
        ...
        let autoGroup = config?.nodes.first(where: { $0.name == "Auto" })
        assert(autoGroup?.isGroup == true, "IsGroup", "Auto should be a group")
        assert(autoGroup?.groupType == "url-test", "GroupType", "Auto should be url-test")
        
        let selectGroup = config?.nodes.first(where: { $0.name == "Select" })
        assert(selectGroup?.proxies?.contains("Auto") == true, "Hierarchy", "Select group should contain Auto group")
    }

    func testVMessLink() {
        print("\n--- Test: VMess Link ---")
        // {"v":"2","ps":"LinkNode","add":"addr","port":123,"id":"uuid","net":"ws","path":"/path","tls":"tls"}
        ...
        assert(config?.nodes.first?.name == "LinkNode", "Name", "Should decode name correctly")
    }
...
"""
        ...
        assert(config?.rules?.first?.type == .domain, "RuleType", "First rule: domain")
        assert(config?.rules?.last?.type == .final, "RuleType", "Last rule: match->final")
    }
...
"""
        ...
        assert(config?.nodes.last?.name == "Node With Emoji 🚀", "EmojiInName", "Should handle emojis in name")
    }
